(() => {
  ...
  function addStyles() {
    ...
    const style = document.createElement("style"); style.id = "rizney-animal-styles"; style.textContent = `
      ...
      .wat-board{display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(2,1fr);gap:10px;width:100%;max-width:500px;margin:8px auto 0;padding:10px;border:2px solid var(--gold);border-radius:10px;box-sizing:border-box;background:#000}
  ...
  function updateHealth() { if (!game) return; game.healthFill.style.width = `${Math.max(0, health) / 6 * 100}%`; game.healthText.textContent = `Hits remaining: ${health}`; }
  ...
  function finish(won) { active = false; clearTimeout(moleTimer); clearInterval(gameTimer); hideMoles(); if (!game) return; if (won) { game.status.textContent = "TRACK COMPLETE!"; setTimeout(() => { closeGame(); $("#next-song")?.click(); }, 500); } else { game.status.textContent = "Time ran out."; setTimeout(closeGame, 700); } }
  ...
  function spawnMole() { if (!active || !game) return; hideMoles(); const hole = [...game.board.children][Math.floor(Math.random() * game.board.children.length)]; if (!hole) return; hole.dataset.active = "true"; hole.textContent = "🐾"; clearTimeout(moleTimer); moleTimer = setTimeout(() => { if (!active || hole.dataset.active !== "true") return; hole.dataset.active = "false"; hole.textContent = "🕳️"; }, 1200); }
  function createGame() { if (game) return game; const panel = document.createElement("section"); panel.id = "whack-a-track-game"; panel.hidden = true; panel.innerHTML = `<div class="wat-health"><div class="wat-health-fill"></div></div><p class="wat-timer"></p><p id="wat-status"></p><div class="wat-board"></div>`; const board = $(".wat-board", panel), status = $("#wat-status", panel); const healthFill = $(".wat-health-fill", panel), healthText = status, timer = $(".wat-timer", panel); for (let i = 0; i < 6; i++) { const hole = document.createElement("button"); hole.type = "button"; hole.className = "wat-hole"; hole.dataset.active = "false"; hole.textContent = "🕳️"; hole.addEventListener("click", () => whack(hole)); board.appendChild(hole); } const parent = controls(); if (parent) parent.insertAdjacentElement("afterend", panel); else ($("main") || document.body).prepend(panel); game = { panel, board, status, healthFill, healthText, timer }; return game; }
  function toggleGame(event) { event.preventDefault(); event.stopPropagation(); if (active || (game && !game.panel.hidden)) { closeGame(); return; } game = createGame(); syncGamePosition(); game.panel.hidden = false; health = 6; seconds = 60; active = true; updateHealth(); updateTimer(); game.status.textContent = "Whack all the moles!"; clearInterval(gameTimer); gameTimer = setInterval(() => { seconds -= 1; updateTimer(); if (seconds <= 0) finish(false); else spawnMole(); }, 1000); spawnMole(); }
