(() => {
  ...
  const animals = [
    "aardvark","alligator","ant","anteater","armadillo","baboon","badger","bat","bear","beaver","bee","bobcat","camel","cat","dolphin","eagle","fox","frog","giraffe","goat","hawk","otter","owl","panda","rabbit","raccoon","sheep","shark","whale","wolf","zebra"
  ];
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .wat-meter{width:min(100%,360px);margin:4px auto 6px;text-align:left}
      .wat-meter-label{display:block;margin-bottom:2px;color:var(--bright-gold);font-size:.68rem;font-weight:bold;letter-spacing:.06em}
      .wat-health,.wat-time{height:10px;width:100%;margin:0;border:1px solid var(--gold);border-radius:999px;background:#000;overflow:hidden}
  ...
  function syncGamePosition() {
    const bar = controls();
    if (bar) document.documentElement.style.setProperty("--whack-toolbar-bottom", `${Math.max(0, bar.getBoundingClientRect().bottom)}px`);
  }
  ...
  function updateTimer() {
    ...
    game.timer.textContent = `Moles: ${whacks}/${targetWhacks}`;
  }
  ...
  function createGame() {
    ...
    panel.innerHTML = `
      <div class="wat-meter">
        <span class="wat-meter-label">MOLE HEALTH</span>
        <div class="wat-health"><div class="wat-health-fill"></div></div>
      </div>
      <div class="wat-meter">
        <span class="wat-meter-label">TIME REMAINING</span>
